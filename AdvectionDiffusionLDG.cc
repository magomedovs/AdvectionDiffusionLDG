...
namespace LDG
{
  ...
  void AdvectionDiffusionProblem<dim>::assemble_system()
  {
    ...
    const auto cell_worker = [&](const auto &cell, auto &scratch_data, auto &copy_data) {
      ...
      advection->value_list(q_points, advection_values);

      std::vector<double> diffusion_values(n_q_points);
      diffusion->value_list(q_points, diffusion_values);
        ...
        for (unsigned int i = 0; i < fe_v.dofs_per_cell; ++i)
        {
          for (unsigned int j = 0; j < fe_v.dofs_per_cell; ++j)
          {
            copy_data.cell_matrix(i, j) += (
              1./diffusion_values[point] * phi_sigma[i] * phi_sigma[j] +
              -div_phi_sigma[i] * phi_u[j] +
              -grad_phi_u[i] * advection_values[point] * phi_u[j] +
    ...
    const auto boundary_worker = [&](const auto         &cell,
      ...
      std::vector<Tensor<1, dim>> Neumann_boundary_values(n_q_points);
      Neumann_boundary_flux_function->value_list(q_points, Neumann_boundary_values);
      ...
      for (unsigned int point = 0; point < n_q_points; ++point)
      {
        ...
        for (unsigned int i = 0; i < dofs_per_cell; ++i)
        {
          ...
          {     
            ...
            {
              copy_data.cell_matrix(i, j) += (
                phi_u[i] * (phi_sigma[j] * normals[point] + diffusion_values[point] * eta * (phi_u[j]))
              ) * JxW[point];                // dx
            }       
          ...
          if (cell->face(face_no)->boundary_id() == Dirichlet)
          {
            copy_data.cell_rhs(i) += (
              ...
              -phi_u[i] * Dirichlet_boundary_values[point] * advection_values[point] * normals[point]
              + phi_u[i] * diffusion_values[point] * eta * Dirichlet_boundary_values[point]
            ) * JxW[point];                // dx
          }
          else if (cell->face(face_no)->boundary_id() == Neumann)
          {
            copy_data.cell_rhs(i) += (              
              -phi_u[i] * Neumann_boundary_values[point] * normals[point]        
            ) * JxW[point];                // dx
          }
          else
          {
    ...
    const auto face_worker = [&](const auto         &cell,
      ...
      {        
        ...
        const double advection_dot_n = advection_values[point] * normals[point];
        ...
        for (const unsigned int i : fe_iv.dof_indices())
        {
          for (const unsigned int j : fe_iv.dof_indices())
          {
            copy_data_face.cell_matrix(i, j) += (
              ...
              phi_u_jump[i] * (phi_sigma_avg[j] * normals[point] + beta * normals[point] * phi_sigma_jump[j] * normals[point] + diffusion_values[point] * eta * phi_u_jump[j])
            ) * JxW[point];                                 // dx
          }
        }
      }
